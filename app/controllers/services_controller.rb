class ServicesController < ApplicationController
  ...
  def destroy
    ...
    if session[:service_id] == @service.id
      flash[:error] = 'Você está atualmente logado com essa conta!'
    else
      @service.destroy
    end
  ...
  def newaccount
    if params[:commit] == "Cancel"
      ...
      if @newuser.save!
        ...
        flash[:notice] = 'Sua conta foi criada e voce está autenticado!'
        redirect_to root_url
      else
  ...
  def signout 
    if current_user
      ...
      flash[:notice] = 'Sua sessão foi terminada!'
    end  
    redirect_to root_url
  end
  ...
  def create
    ...
    if omniauth and params[:service]
      ...
      if @authhash[:uid] != '' and @authhash[:provider] != ''
        ...
        if user_signed_in?
          if auth
            flash[:notice] = 'A conta ' + @authhash[:provider].capitalize + ' Ja está associada a este aplicativo.'
            redirect_to services_path
          else
            current_user.services.create!(:provider => @authhash[:provider], :uid => @authhash[:uid], :uname => @authhash[:name], :uemail => @authhash[:email])
            flash[:notice] = 'A conta ' + @authhash[:provider].capitalize + ' foi adicionada para autenticação neste aplicativo.'
            redirect_to services_path
          end
        else
          if auth
            ...
            flash[:notice] = 'Login efetuado via ' + @authhash[:provider].capitalize + '.'
            redirect_to root_url
          else
        ...
        flash[:error] =  'Erro autenticando via ' + service_route + '/' + @authhash[:provider].capitalize + '. The service returned invalid data for the user id.'
        redirect_to signin_path
      end
    else
      flash[:error] = 'Erro autenticando via ' + service_route.capitalize + '. The service did not return valid data.'
      redirect_to signin_path
    end
  end
